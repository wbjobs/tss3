...
fn spikyKernelGradient(r: f32, h: f32) -> f32 {
    ...
    let hr: f32 = h - r;
    return -(45.0 / (PI * pow(h, 6.0))) * hr * hr;
}
...
fn hashGridIndex(cell: vec2<i32>) -> u32 {
    let p1: u32 = 73856093u;
    let p2: u32 = 19349663u;
    let res: u32 = params.gridResolution * params.gridResolution;
    let h: u32 = (u32(cell.x) * p1) ^ (u32(cell.y) * p2);
    return h % res;
}
...
fn computeDensityPressure(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let cell: vec2<i32> = getGridCell(pi.position);
    ...
    for (var dx: i32 = -1; dx <= 1; dx++) {
        for (var dy: i32 = -1; dy <= 1; dy++) {
            ...
            for (var k: u32 = start; k < end; k++) {
                ...
                let diff: vec2<f32> = pi.position - pj.position;
                let rSq: f32 = dot(diff, diff);
                
                if (rSq < hSq) {
                    let h2MinusR2: f32 = hSq - rSq;
                    density += poly6Factor * h2MinusR2 * h2MinusR2 * h2MinusR2;
                }
            }
        }
    }
    ...
    pi.density = density;
    pi.pressure = params.gasConstant * (density - params.restDensity);
    particles[i] = pi;
}
...
fn computeForces(@builtin(global_invocation_id) gid: vec3<u32>) {
        ...
        for (var dy: i32 = -1; dy <= 1; dy++) {
            ...
            for (var k: u32 = start; k < end; k++) {
                ...
                if (rSq < hSq && rSq > 0.000001) {
                    ...
                    let gradScale: f32 = spikyFactor * hMinusR * hMinusR / r;
                    let pressureTerm: f32 = (pi.pressure + pj.pressure) / (2.0 * max(pj.density, 0.001));
                    pressureForce += pressureTerm * gradScale * diff;
                    
                    // Viscosity kernel laplacian
                    let laplacian: f32 = viscosityFactor * hMinusR;
    ...
    pi.velocity = pi.velocity + acceleration * params.dt;
    
    let speedSq: f32 = dot(pi.velocity, pi.velocity);
    let speed: f32 = sqrt(speedSq);
    let t: f32 = clamp(speed / 5.0, 0.0, 1.0);
    pi.color = mix(vec3<f32>(0.2, 0.5, 1.0), vec3<f32>(1.0, 0.3, 0.2), t);
...
fn integrate(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var o: u32 = 0u; o < 64u; o = o + 1u) {
        ...
        if (dist < minDist) {
            let dir: vec2<f32> = normalize(diff + vec2<f32>(0.0001, 0.0));
            p.position = obs.position + dir * minDist;
            
            let velAlongNormal: f32 = dot(p.velocity, dir);
            if (velAlongNormal < 0.0) {
                p.velocity = p.velocity - 1.5 * velAlongNormal * dir;
            }
        }
    }
    ...
    if (params.mouseActive == 1u && params.mouseMode == 0u) {
        ...
        if (dist < 0.15) {
            ...
            p.velocity = p.velocity + normalize(diff + vec2<f32>(0.0001, 0.0)) * force * params.dt;
        }
    }
...
fn addParticles(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (params.mouseActive == 1u && params.mouseMode == 0u && params.particleCount < 8192u) {
        var p: Particle = particles[i];
        if (p.density < 0.0) {
